...
namespace endian
{
template <typename T, eEndian TO_E, std::size_t N = sizeof(T)>
...
constexpr decltype(auto) create_store_funcs_array(std::index_sequence<I...>)
{
    ...
    return std::array<store_func, sizeof...(I)>{store<T, TO_E, I + 1>...};
}
...
template <typename T, eEndian TO_E, std::size_t N>
inline constexpr void store(std::byte *aBuffer, const T &aValue) noexcept
{
    assert(aBuffer);
    static_assert(N > 0);
